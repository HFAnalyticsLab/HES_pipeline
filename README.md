...
Service (DARS)](https://digital.nhs.uk/services/data-access-request-service-dars) and development of this pipeline is now underway.
...
* reads HES raw data files in chunks and before adding it to the database, 
    + checks if all expected columns are present
    + coerces data types
    + cleans variables 
    + derives variables (based on a single row of data)
    + combines with public data on deprivation and CCGs (not yet implemented)
    + flags comorbidities (not yet implemented)
* flags duplicates in the database (in progress, not yet functional)
...
The architecture design record (ADR) captures architectural decisision and design 
...
## Usage: running the pipeline on a HES extract
